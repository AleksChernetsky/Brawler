        ...
        while (true)
        {
            ...
            for (int i = 0; i < hits; i++)
            {
                if (NavMesh.SamplePosition(Colliders[i].transform.position, out NavMeshHit hit, 2f, Agent.areaMask))
                {
                    ...
                    var dotprod = Vector3.Dot(hit.normal, (Target.position - hit.position).normalized);
                    if (dotprod < HideSensitivity)
                    {
                    ...
                    else
                    {
                        ...
                        if (NavMesh.SamplePosition(Colliders[i].transform.position - (Target.position - hit.position).normalized * 2, out NavMeshHit hit2, 2f, Agent.areaMask))
                        {
                            ...
                            Debug.DrawRay(hit2.position, Vector3.up, Color.blue, 3f);
